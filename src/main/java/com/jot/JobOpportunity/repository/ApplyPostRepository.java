...
public interface ApplyPostRepository extends JpaRepository<ApplyPost, Long> {
    ...
    @Query(value = "SELECT a FROM ApplyPost a WHERE a.posterId = :posterId ORDER BY a.postId ASC")
    List<ApplyPost> getByPosterId(@Param("posterId") Long posterId);
    ...
    @Query(value = "SELECT ap.id as Id, ap.account_id as accountId, ap.app_time as appTime, ap.link_cv as linkCv,\n" +
            ...
            "WHERE ap.account_id = :id", nativeQuery = true)
